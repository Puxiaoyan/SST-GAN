...
class PoolHiddenNet(nn.Module):
    ...
    def forward(self, h_states, seq_start_end, end_pos):
        ...
        for _, (start, end) in enumerate(seq_start_end):
            ...
            clustering.fit(curr_end_pos)
            labels = clustering.labels_
            cluster_centers = clustering.cluster_centers_
            clustered_data = [curr_end_pos[labels == label] for label in np.unique(labels)]
            clusters_indices = [np.where(labels == i)[0] for i in range(len(cluster_centers))]
            for i, group_data in enumerate(clustered_data):
                curr_hidden = curr_hidden[clusters_indices[i]]
                curr_end_pos = group_data
...
class FeatureFusion(nn.Module):
    ...
    def forward(self, feat1, feat2):
        if self.mode == 'sum':
            ...
            cnn = nn.Sequential(
                nn.Conv2d(in_channels=2, out_channels=16, kernel_size=5, stride=2, padding=0),
                nn.ReLU(),
                nn.Conv2d(in_channels=16, out_channels=32, kernel_size=5, stride=2, padding=0),
                nn.ReLU(),
                nn.Conv2d(in_channels=32, out_channels=feat1.shape[1], kernel_size=5, stride=2, padding=0),
                nn.Tanh()
            )
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            cnn.to(device)
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, teacher_forcing_ratio=0.5, training_step=3, user_noise=None):
        ...
        if training_step == 2:
            ...
            graph_lstm_input = self.fusion(graph_lstm_input, torch.stack(pool_output_all))
