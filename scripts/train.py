...
from sstgan.data.loader import data_loader
from sstgan.losses import gan_g_loss, gan_d_loss, l2_loss
from sstgan.losses import displacement_error, final_displacement_error
from sstgan.losses import AverageMeter, ProgressMeter

from sstgan.models import TrajectoryGenerator, TrajectoryDiscriminator
from sstgan.utils import int_tuple, bool_flag, get_total_norm
...
parser.add_argument('--loader_num_workers', default=4, type=int)  # 没用到
...
parser.add_argument('--pooling_ratio', default=0.5)
...
parser.add_argument('--d_learning_rate', default=1e-2, type=float)
    ...
    while t < args.num_iterations:
        ...
        if training_step != 3:
            optimizer = optim.Adam(
                [
                    {"params": generator.traj_lstm_model.parameters(), "lr": 1e-2},
                    {"params": generator.traj_hidden2pos.parameters()},
                    {"params": generator.gatencoder.parameters(), "lr": 3e-2},
                    {"params": generator.graph_lstm_model.parameters(), "lr": 1e-2},
                    {"params": generator.traj_gat_hidden2pos.parameters()},
                    {"params": generator.pred_lstm_model.parameters()},
                    {"params": generator.pred_hidden2pos.parameters()},
                ],
                lr=args.lr,
            )
            train(args, generator, train_loader, optimizer, epoch, training_step)
        else:
...
def train(args, model, train_loader, optimizer, epoch, training_step):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        pred_traj_fake_rel = model(
            obs_traj, model_input, seq_start_end, 1, training_step
        )
        l2_loss_rel.append(
...
def discriminator_step(
    ...
    generator_out = generator(obs_traj, model_input, seq_start_end, 0)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            pred_traj_fake_rel = generator(
                obs_traj, model_input, seq_start_end
            )
            pred_traj_fake = relative_to_abs(pred_traj_fake_rel, obs_traj[-1])
